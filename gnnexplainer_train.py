# This file is copied from GNNExplainer repo with small modifications
...
import gnnexplainer_configs
import gnnexplainer_gengraph

import gnnexplainer_utils.math_utils as math_utils
import gnnexplainer_utils.io_utils as io_utils
import gnnexplainer_utils.parser_utils as parser_utils
import gnnexplainer_utils.train_utils as train_utils
import gnnexplainer_utils.featgen as featgen
import gnnexplainer_utils.graph_utils as graph_utils
        ...
        for batch_idx, data in enumerate(dataset):
            ...
            if epoch == args.num_epochs - 1:
                if batch_idx == 0:
                    ...
                    all_idx = torch.cat((all_idx, prev_idx), dim=0)
...
def train_node_classifier(G, labels, model, args, writer=None):
    ...
    data = gnnexplainer_gengraph.preprocess_input_graph(G, labels)
...
def train_node_classifier_multigraph(G_list, labels, model, args, writer=None):
    ...
    data = gnnexplainer_gengraph.preprocess_input_graph(G_list[0], labels[0])
    ...
    for i in range(1, len(G_list)):
        ...
        test_idx = idx[num_train:]
        test_idx_all.append(test_idx)
        data = gnnexplainer_gengraph.preprocess_input_graph(G_list[i], labels[i])
...
def syn_task1(args, writer=None):
    # data
    G, labels, name = gnnexplainer_gengraph.gen_syn1(
...
def syn_task2(args, writer=None):
    # data
    G, labels, name = gnnexplainer_gengraph.gen_syn2()
...
def syn_task3(args, writer=None):
    # data
    G, labels, name = gnnexplainer_gengraph.gen_syn3(
        feature_generator=featgen.ConstFeatureGen(np.ones(args.input_dim, dtype=float))
    )
    print(labels)
    num_classes = max(labels) + 1
...
def syn_task4(args, writer=None):
    # data
    G, labels, name = gnnexplainer_gengraph.gen_syn4(
        feature_generator=featgen.ConstFeatureGen(np.ones(args.input_dim, dtype=float))
    )
    print(labels)
    num_classes = max(labels) + 1
...
def main():
    prog_args = gnnexplainer_configs.arg_parse()
