...
class Explainer:
    ...
    # Main method
    def explain(
        self, node_idx, graph_idx=0, graph_mode=False, unconstrained=False, model="exp",original_idx = None
    ):
        ...
        # with open('sidelog/%d.%d.adj.npy' % (original_idx, pred_label), 'wb') as outfile:
        #     np.save(outfile, np.asarray(sub_adj.copy()))
        final_node_embeddings = self.model.final_node_embeddings(x, adj).detach().numpy()[0]
        with open('embeddings-%s/%d.%d.embs.npy' % (self.args.bmname, original_idx, pred_label), 'wb') as outfile:
            np.save(outfile, np.asarray(final_node_embeddings.copy()))
        ...
        if model == "grad":
            ...
            for epoch in range(self.args.num_epochs+1):
                ...
                if self.writer is not None:
                    ...
                    # comment the following lines when mass explaining
                    # if epoch % 25 == 0:
        ...
        with open('explanations/gnnexplainer/%d.%d.masked_adj.npy' % (original_idx, pred_label), 'wb') as outfile:
